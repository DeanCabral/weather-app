...
export default function MapLegend({ mapType }: Props) {
    ...
    return (
        <div className="absolute top-4 right-4 w-96 rounded-xl shadow-lg p-4 bg-background/50 border-accent/70 flex flex-col gap-4">
            <h3 className="text-sm font-semibold text-foreground">{data.title}</h3>
            <div className="width-full rounded-xl border-accent/70" style={{ background: `linear-gradient(to right, ${gradientStops})`, height: '20px' }} />
...
const mapTypeData: Record<string, { title: string; unit: string; stops: { value: number; color: string }[] }> = {
    clouds_new: {
        title: "Clouds (%)",
        unit: "%",
        stops: [
